...
export default function PlayerList({ route, navigation }) {
    const { teamID, playerList, tournamentID , submit, setSubmit, eventID } = route.params
    ...
    const approveRegistration =async () =>{
        try {
            const res = await axios.post(`/${eventID}/${tournamentID}/${teamID}/approveTeamRegistration`)
    ...
    const rejectRegistration =async () =>{
        try {
            const res = await axios.post(`/${eventID}/${tournamentID}/${teamID}/rejectTeamRegistration`)
            console.log(res.data)
        } catch (error) {
                ...
                {playerList && playerList.map((item, i) => {
                    return (
                        <TouchableOpacity
                            key={i}
                            onPress={() => {
                                navigation.navigate('PlayerDetailsApproval', { playerData: item, tournamentID, eventID })
                            }}>
                            <DataTable.Row key={i}>
